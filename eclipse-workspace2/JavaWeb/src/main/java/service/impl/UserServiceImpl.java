...
public class UserServiceImpl implements UserService {
	...
	public void changePasswordById(Integer id, String oldPassword, String newPassword) {
		...
		String oldHash = user.getHash();//原始哈希(目前存放在資料表的哈希)
		//oldPassword+oldSalt== oldHash
		if(!PasswordHash.getHashPassword(oldPassword, oldSalt).equals(oldHash)) {
			throw new RuntimeException("當密碼輸入錯誤");
		}
		
		//進行新密碼更改
		String newSalt = PasswordHash.generateSalt();//取的新鹽
	...
	public boolean login(String username, String password) {
		...
		String salt = user.getSalt();//該使用者目前存放在資料中的鹽
		String hash = user.getHash();//該使用者目前存放在資料中的哈希
		//判斷
